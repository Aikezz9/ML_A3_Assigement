...
from model.loaded_model3 import *
...
def A3():
    model3 = load_model3()
    scaler, year_default, max_power_default,mileage_default,brand_ohe, classes, poly, years = load_data3()
    return render_template('A3.html',brand_ohe=brand_ohe.categories_[0], year_default=year_default, max_power_default=max_power_default, mileage_default=mileage_default, years=sorted(years))
...
def a3_predict():
    ...
    predicted_result = model3.predict(input.astype(float))
    return str(classes[int(predicted_result[0])])
